...
count = 0
userbot = None
timeout_duration = 60  # 1 minute default
...
async def txt_handler(bot: Client, m: Message):  
    ...
    # ✅ FIX: Channel ID properly handle aur integer mein convert karo
    if "/d" in raw_text7:
        ...
        try:
            ...
            await m.reply_text(
                "❌ **Invalid Channel ID!**\n\n"
                "Please send a valid numeric channel ID like:\n"
                "`-1001234567890`\n\n"
                "Try /drm again with correct channel ID."
            )
            return

    # ✅ FIX: Verify if bot is admin in target channel
        ...
        try:
            ...
            if not bot_member.status == "administrator":
                await m.reply_text(
                    ...
                    "Please make me admin with following permissions:\n"
                    "• Post Messages\n• Edit Messages\n• Delete Messages\n• Pin Messages\n\n"
                    "Then try again with /drm"
                )
                return
            if bot_member.privileges and not bot_member.privileges.can_post_messages:
                await m.reply_text("❌ I don't have **Post Messages** permission in that channel!")
                return
        except ChatAdminRequired:
            await m.reply_text("❌ I am not admin in that channel. Please add me as admin first.")
            return
        except PeerIdInvalid:
            await m.reply_text("❌ Invalid channel ID. Bot has never seen this chat before.")
            return
        except Exception as e:
            await m.reply_text(f"❌ Cannot access channel: `{e}`")
            return

    await editable.delete()

    # ✅ FIX: Broken Pin/Delete Logic fixed
    try:
        if raw_text == "1":
            ...
            try:
                ...
                print(f"Pin failed (not critical): {pin_err}")
        else:
            if channel_id != m.chat.id:
                await bot.send_message(
                    chat_id=m.chat.id,
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "acecwply" in url:
            ...
            elif "againbwapis.vercel.app" in url or "rupkama.vercel.app" in url:
                max_retries = 3
                api_success = False
                raw_url = url  # Default fallback
                            ...
                            async with session.get(url, timeout=60) as resp:
                                if resp.status == 200:
                                    ...
                                    if data.get("url"):
                                        raw_url = data["url"]
                                        api_success = True
                                        print(f"✅ API Success on Attempt {attempt}")
                                        break
                                    else:
                                        print(f"⚠️ Attempt {attempt} - No URL in response: {data}")
                                else:
                                    print(f"⚠️ Attempt {attempt} - Status: {resp.status}")
                    except Exception as e:
                ...
                if not api_success or not raw_url:
                    ...
                    await bot.send_message(
                        ...
                        f'<blockquote><i><b>Failed Reason: API Error after {max_retries} retries</b></i></blockquote>',
                ...
                # Split URL and Keys (Same format as rest of the code)
                if "*" in raw_url:
                    video_url = raw_url.split("*", 1)[0]
                    key_part = raw_url.split("*", 1)[1]
                ...
                url = video_url  # Reassign url
                mpd = video_url if ".mpd" in video_url else None
            ...
            try:
                ...
                ccimg = (
                    ...
                    f"<b>🖼️  Tɪᴛʟᴇ</b> : {name1} \n\n"
                    f"<blockquote>📚  𝗕𝴀ᴛᴄʜ : {b_name}</blockquote>"
                    f"\n\n<b>🎓  Uᴘʟᴏᴀᴅ Bʏ : {CR}</b>"
                )
